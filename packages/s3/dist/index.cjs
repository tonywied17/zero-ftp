...
function createMemoryS3MultipartResumeStore() {
  const map = /* @__PURE__ */ new Map();
  const stringify = (key) => `${key.bucket}\0${key.jobId}\0${key.path}`;
  return {
    clear: (key) => {
...
var S3Provider = class {
  ...
  async connect(profile) {
    ...
    const sessionToken = this.internals.sessionToken !== void 0 ? secretToString(await resolveSecret(this.internals.sessionToken)) : void 0;
