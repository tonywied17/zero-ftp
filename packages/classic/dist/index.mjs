...
function parseKnownHosts2(source) {
  ...
  const entries = [];
  for (const value of values) {
    const text = Buffer7.isBuffer(value) ? value.toString("utf8") : value;
    const lines = text.split(/\r?\n/);
...
function hashedKnownHostPatternMatches(pattern, candidate) {
  ...
  const salt = Buffer7.from(saltText, "base64");
  const expected = Buffer7.from(hashText, "base64");
...
function normalizeHostKeyPin(value) {
  ...
  if (hex.length === 64 && /^[a-f0-9]+$/i.test(hex)) {
    return Buffer7.from(hex, "hex").toString("base64").replace(/=+$/g, "");
  }
  const bare = trimmed.startsWith("SHA256:") ? trimmed.slice("SHA256:".length) : trimmed;
  return Buffer7.from(padBase642(bare), "base64").toString("base64").replace(/=+$/g, "");
}
...
async function* createSftpReadSource(sftp, path2, range, request) {
  ...
  try {
    for await (const chunk of stream) {
      request.throwIfAborted();
      yield new Uint8Array(Buffer7.from(chunk));
    }
  } catch (error) {
...
function writeSftpChunk(stream, chunk) {
  ...
  return new Promise((resolve, reject) => {
    ...
    stream.write(Buffer7.from(chunk), (error) => {
      cleanup();
      if (error != null) {
        reject(error);
        return;
...
function normalizeBastionConfig(config) {
  ...
  if (typeof cloned.privateKey === "string") {
    cloned.privateKey = Buffer8.from(cloned.privateKey);
  }
  return cloned;
}
export {
