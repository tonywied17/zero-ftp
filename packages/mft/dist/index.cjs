...
__export(mft_exports, {
  ...
  summarizeError: () => summarizeError2,
...
// src/mft/conventions.ts
var DEFAULT_PROCESSED_SUBDIR = "processed";
var DEFAULT_FAILED_SUBDIR = "failed";
function inboxProcessedPath(inbox) {
  return joinRemotePath(inbox.basePath, inbox.processedSubdir ?? DEFAULT_PROCESSED_SUBDIR);
}
function inboxFailedPath(inbox) {
  return joinRemotePath(inbox.basePath, inbox.failedSubdir ?? DEFAULT_FAILED_SUBDIR);
}
function createInboxRoute(options) {
  const route = {
  ...
  if (options.name !== void 0) route.name = options.name;
  if (options.description !== void 0) route.description = options.description;
  if (options.operation !== void 0) route.operation = options.operation;
  if (options.enabled !== void 0) route.enabled = options.enabled;
  if (options.inbox.filter !== void 0) route.filter = options.inbox.filter;
  return route;
}
function createOutboxRoute(options) {
  const route = {
...
// src/mft/retention.ts
function evaluateRetention(options) {
  validatePolicy(options.policy);
  if (options.policy.kind === "age") {
    return evaluateAgePolicy(options.entries, options.policy, options.now ?? /* @__PURE__ */ new Date());
  }
...
function evaluateAgePolicy(entries, policy, now) {
  ...
  for (const entry of entries) {
    ...
    if (modifiedAt === void 0 || modifiedAt >= cutoff) {
      keep.push(entry);
    } else {
      evict.push(entry);
    }
...
function evaluateCountPolicy(entries, policy) {
  const files = [];
  const passthrough = [];
  for (const entry of entries) {
    if (entry.type === "file") {
  ...
  const sorted = [...files].sort((a, b) => compareEntries5(a, b, sortBy));
...
function compareEntries5(a, b, sortBy) {
  if (sortBy === "name") {
...
function createJsonlAuditLog(writer) {
  return {
...
function composeAuditLogs(...logs) {
  return {
    list: async () => {
...
function signWebhookPayload(payload, secret, timestamp = (/* @__PURE__ */ new Date()).toISOString()) {
  const mac = (0, import_node_crypto2.createHmac)("sha256", secret);
  mac.update(`${timestamp}.${payload}`);
  return { digest: mac.digest("hex"), timestamp };
}
async function dispatchWebhook(options) {
  ...
  while (attempt < retry.maxAttempts) {
    ...
    if (options.target.secret !== void 0) {
...
function createWebhookAuditLog(options) {
  return {
    list: () => Promise.resolve([]),
    record: async (entry) => {
      if (options.target.types !== void 0 && !options.target.types.includes(entry.type)) {
        return;
      }
      const dispatchOptions = { payload: entry, target: options.target };
      if (options.fetch !== void 0) dispatchOptions.fetch = options.fetch;
      if (options.retry !== void 0) dispatchOptions.retry = options.retry;
      if (options.sleep !== void 0) dispatchOptions.sleep = options.sleep;
...
function defaultSleep2(delayMs) {
  return new Promise((resolve) => setTimeout(resolve, delayMs));
}

// src/mft/approvals.ts
var ApprovalRejectedError = class _ApprovalRejectedError extends ZeroTransferError {
  /**
  ...
  constructor(request) {
    ...
    Object.setPrototypeOf(this, _ApprovalRejectedError.prototype);
...
var ApprovalRegistry = class {
   ...
   */
  create(input, now = /* @__PURE__ */ new Date()) {
    ...
    if (input.metadata !== void 0) request.metadata = input.metadata;
   ...
   */
  approve(id, input = {}, now = /* @__PURE__ */ new Date()) {
   ...
   * @returns The updated approval record.
   */
  reject(id, input = {}, now = /* @__PURE__ */ new Date()) {
  ...
  resolve(id, status, input, now) {
    ...
    if (input.resolvedBy !== void 0) request.resolvedBy = input.resolvedBy;
    if (input.reason !== void 0) request.reason = input.reason;
    if (status === "approved") {
      const resolver = this.pending.get(id);
      if (resolver !== void 0) {
        this.pending.delete(id);
...
function nextCronFireAt(expression, from, timezone = "utc") {
  const utc = timezone === "utc";
  const start = new Date(from.getTime() + 6e4 - from.getTime() % 6e4);
  const limit = start.getTime() + 5 * 365 * 24 * 60 * 60 * 1e3;
  let candidate = start;
  while (candidate.getTime() <= limit) {
    ...
    if (expression.minute.includes(minute) && expression.hour.includes(hour) && expression.month.includes(month) && matchesDayOfMonthAndWeek(expression, dom, dow)) {
      return candidate;
    }
...
function matchesDayOfMonthAndWeek(expression, dom, dow) {
...
function parseField(raw, min, max, name, expression) {
  ...
  for (const part of raw.split(",")) {
    ...
    const step = stepRaw === void 0 ? 1 : parseStep(stepRaw, name, expression);
    let rangeStart;
    let rangeEnd;
    if (range === "*") {
      rangeStart = min;
      ...
      rangeEnd = stepRaw === void 0 ? rangeStart : max;
    }
...
function parseStep(raw, name, expression) {
...
function normalizeDayOfWeek(values) {
...
function createCronError(field, expression) {
  return new ConfigurationError({
...
function nextScheduleFireAt(schedule, from = /* @__PURE__ */ new Date()) {
...
function compileTrigger(schedule) {
...
function nextIntervalFireAt(trigger, from) {
...
// src/mft/MftScheduler.ts
var MftScheduler = class {
  options;
  now;
  setTimer;
  clearTimer;
  ...
  constructor(options) {
    ...
    this.now = options.timer?.now ?? (() => /* @__PURE__ */ new Date());
    this.setTimer = options.timer?.setTimer ?? ((callback, delayMs) => setTimeout(callback, delayMs));
    this.clearTimer = options.timer?.clearTimer ?? ((handle) => clearTimeout(handle));
  }
...
function withIntervalAnchor(schedule, reference) {
  if (schedule.trigger.kind !== "interval" || schedule.trigger.anchor !== void 0) {
    return schedule;
  }
  return {
    ...schedule,
...
var defaultRunner = ({ client, route, signal }) => {
