...
function parseKnownHosts2(source) {
  ...
  const entries = [];
  for (const value of values) {
    const text = import_node_buffer6.Buffer.isBuffer(value) ? value.toString("utf8") : value;
    const lines = text.split(/\r?\n/);
...
function hashedKnownHostPatternMatches(pattern, candidate) {
  ...
  const salt = import_node_buffer6.Buffer.from(saltText, "base64");
  const expected = import_node_buffer6.Buffer.from(hashText, "base64");
...
function normalizeHostKeyPin(value) {
  ...
  if (hex.length === 64 && /^[a-f0-9]+$/i.test(hex)) {
    return import_node_buffer6.Buffer.from(hex, "hex").toString("base64").replace(/=+$/g, "");
  }
  const bare = trimmed.startsWith("SHA256:") ? trimmed.slice("SHA256:".length) : trimmed;
  return import_node_buffer6.Buffer.from(padBase642(bare), "base64").toString("base64").replace(/=+$/g, "");
}
...
async function* createSftpReadSource(sftp, path2, range, request) {
  ...
  try {
    for await (const chunk of stream) {
      request.throwIfAborted();
      yield new Uint8Array(import_node_buffer6.Buffer.from(chunk));
    }
  } catch (error) {
...
function writeSftpChunk(stream, chunk) {
  ...
  return new Promise((resolve, reject) => {
    ...
    stream.write(import_node_buffer6.Buffer.from(chunk), (error) => {
      cleanup();
      if (error != null) {
        reject(error);
        return;
...
function normalizeBastionConfig(config) {
  ...
  if (typeof cloned.privateKey === "string") {
    cloned.privateKey = import_node_buffer7.Buffer.from(cloned.privateKey);
  }
  return cloned;
}
